...
func (b backupJob) Run() {
	...
	if err != nil {
		...
		if err := notifier.SendNotification(b.plan.Name, fmt.Sprintf("%v backup failed", b.plan.Name),
		...
		if err := notifier.SendNotification(b.plan.Name, fmt.Sprintf("%v backup finished", b.plan.Name),
